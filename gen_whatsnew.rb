...
works_curr_year = works_sorted.select{ |work| work[:published_on].start_with?(curr_year) }
works_prev_year = works_sorted.select{ |work| work[:published_on].start_with?(prev_year) }

File.write("whatsnew#{curr_year}.json", JSON.dump(works_curr_year)+"\n")
File.write("whatsnew#{prev_year}.json", JSON.dump(works_prev_year)+"\n")
